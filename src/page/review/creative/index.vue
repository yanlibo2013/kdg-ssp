        ...
        <el-main>
            ...
            <el-col :span="24" class="margin-bottom-20">
                ...
                <div class="template-bd border-bottom-2">
                    ...
                    <el-select class="filter-input" v-model="form.ad" placeholder="请选择" @change="filterData('2')">
                        <el-option v-for="(item,index) in adData" :key="index" :label="item.value" :value="item.key">
                        </el-option>
                    </el-select>

                    <label for="" class="data-item">创意ID：</label>
                    <el-select class="filter-input" v-model="form.creativeId" placeholder="请选择" @change="filterData('3')">
                        <el-option v-for="(item,index) in creativeData" :key="index" :label="item.value" :value="item.key">
                        </el-option>
                    </el-select>

                    <div class="last-item">
                        <label for="" class="data-item">审核状态：</label>
                        <el-select class="filter-input" v-model="form.reviewStatus" placeholder="请选择" @change="filterData('')">
    		...
    		<span class="select-item ">审核通过</span>
                ...
                <el-table :data="listData.data" style="width: 100%" @selection-change="handleSelectionChange">
                    ...
                    <el-table-column prop="authority" label="素材">
                        <template slot-scope="scope">
                            <a href="javascript:void(0);" class="link-jump" @click="findDetail(scope.row.material)">查看资质</a>
                            <!-- 弹层start -->
                            <el-dialog class="batch-audit-pop audit-file" title="资质文件" :visible.sync="findDetailVisibleImage" width="680px">
                                <el-form ref="form" :model="form" label-width="80px">
                                    <el-form-item label="创意ID">
                                        <label>{{imageList&&imageList.creative_id}}</label>
                                    </el-form-item>
                                    <el-form-item label="广告样式">
                                        <label v-if='imageList&&imageList.style_id=="1"'>单图</label>
                                        <label v-if='imageList&&imageList.style_id=="2"'>单图文</label>
                                        <label v-if='imageList&&imageList.style_id=="3"'>组图</label>
                                        <label v-if='imageList&&imageList.style_id=="4"'>短视频</label>
                                        <label v-if='imageList&&imageList.style_id=="5"'>视频贴片</label>
                                    </el-form-item>
                                    <el-form-item label="image">
                                        <img class="el-carousel-img" :src="imageList&&imageList.img" width="250px" />
                                    </el-form-item>
                                        <el-form-item label="描述">
                                            <label>{{imageList&&imageList.desc}}</label>
                                        </el-form-item>
                                </el-form>
                            </el-dialog>
                            <!-- 弹层end -->
...
export default {
    ...
    methods: {
        ...
        filterData(val) {
            ...
            if (val == "1") {
                this.form.ad = "";
                this.adData = [];
                let subData = this.getSubData(this.searchData, this.form.demand);
                this.adData = subData.length != 0 && _.concat([{
                    key: 0,
                    value: "全部"
            ...
            if (val == "2") {
                this.form.creativeId = "";
                this.creativeData = [];
                let subData1 = this.getSubData(this.getSubData(this.searchData, this.form.demand), this.form.ad);
                this.creativeData = subData1.length != 0 && _.concat([{
                    key: 0,
                    value: "全部"
                }], this.setData(subData1));
        ...
        getList(data) {
            review.getCreativeList(this.header, data).then(res => {
        ...
        onSubmitBatch() {
            ...
            review.reviewCreative(this.header, {
                ids: ids.toString(),
        ...
        getSubData(value, id) {
            ...
            _.forEach(value, val => {
                ...
                if (val.moreLevelVOS) {
                    this.getSubData(val.moreLevelVOS, id)
                }
            });
    ...
    mounted() {
        ...
        common.filterReport(this.header).then(res => {
            ...
            //广告主
            this.adData = _.map(data.advertiserInfoVOS, val => {
                return {
